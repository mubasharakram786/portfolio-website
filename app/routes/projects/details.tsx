...
export async function loader({request,params}:Route.ClientLoaderArgs):Promise<Project>{
    ...
    if(!res.ok) throw new Response('No project details found!')
    const data = res.json();
    return data
}
...
const details = ({loaderData}:Route.ComponentProps) => {
  ...
  return (
    <>
        <Link to='/project' className="flex items-center text-blue-400 hover:text-blue-500 mb-6 transition    "><FaArrowLeft className="mr-2"/>Back to projects</Link>
